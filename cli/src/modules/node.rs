...
impl DefaultSettings for KaspadSettings {
    async fn defaults() -> Vec<(Self, Value)> {
        ...
        if let Ok(binaries) = kaspa_daemon::locate_binaries(&root, "karin").await {
...
impl Default for Node {
    fn default() -> Self {
        Node {
            settings: SettingsStore::try_new("karin").expect("Failed to create node settings store"),
...
impl Node {
    ...
    async fn display_help(self: Arc<Self>, ctx: Arc<KaspaCli>, _argv: Vec<String>) -> Result<()> {
        ctx.term().help(
            &[
                ("select", "Select Karin executable (binary) location"),
                ("version", "Display Karin executable version"),
                ("start", "Start the local Karin node instance"),
                ("stop", "Stop the local Karin node instance"),
                ("restart", "Restart the local Karin node instance"),
                ("kill", "Kill the local Karin node instance"),
                ("status", "Get the status of the local Karin node instance"),
    ...
    async fn select(self: Arc<Self>, ctx: Arc<KaspaCli>, path: Option<String>) -> Result<()> {
        ...
        match path {
            None => {
                let binaries = kaspa_daemon::locate_binaries(root.as_str(), "karin").await?;

                if binaries.is_empty() {
                    tprintln!(ctx, "No karin binaries found");
                } else {
    ...
    pub async fn handle_event(&self, ctx: &Arc<KaspaCli>, event: Event) -> Result<()> {
        ...
        match event {
            ...
            Event::Exit(_code) => {
                tprintln!(ctx, "Karin has exited");
                self.is_running.store(false, Ordering::SeqCst);
                term.refresh_prompt();
            }
            Event::Error(error) => {
                tprintln!(ctx, "{}", style(format!("Karin error: {error}")).red());
                self.is_running.store(false, Ordering::SeqCst);
                term.refresh_prompt();
            }
