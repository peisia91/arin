...
pub enum NetworkIdError {
    #[error("Invalid network name prefix: {0}. The expected prefix is 'karin'.")]
...
///
/// NetworkId is a unique identifier for a karin network instance.
...
impl NetworkId {
    ...
    /// Returns a textual description of the network prefixed with `karin-`
    pub fn to_prefixed(&self) -> String {
        format!("karin-{}", self)
    }

    pub fn from_prefixed(prefixed: &str) -> Result<Self, NetworkIdError> {
        if let Some(stripped) = prefixed.strip_prefix("karin-") {
