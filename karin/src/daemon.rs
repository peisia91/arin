...
pub fn get_app_dir() -> PathBuf {
    #[cfg(target_os = "windows")]
    return get_home_dir().join("karin");
    #[cfg(not(target_os = "windows"))]
    return get_home_dir().join(".karin");
}
...
do you confirm? (answer y/n or pass --yes to the Karin command line to confirm all interactive questions)";
    ...
    {
        let msg =
            "Node database is from a different Karin *DB* version and needs to be fully deleted, do you confirm the delete? (y/n)";
        get_user_approval_or_exit(msg, args.yes);

        info!("Deleting databases from previous Karin version");
